...
interface EditAnalysisFormProps {
  ...
  onSave: (updated: Partial<Analysis>) => Promise<void>;
}

export default function EditAnalysisForm({ initialData, onSave }: EditAnalysisFormProps) {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      router.push(`/dashboard/saved/${initialData._id}`);
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'Failed to save changes');
    } finally {
      setSaving(false);
    }
  }, [formData, initialData._id, onSave, router, validateForm]);
  ...
  ), [formData, errors, saving, handleInputChange, initialData.results]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="mt-8 flex justify-end gap-4">
        <button
          onClick={() => router.push(`/dashboard/saved/${initialData._id}`)}
